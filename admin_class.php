...
class Action
{
	...
	function login()
	{
		...
		if ($qry->num_rows > 0) {
			$row = $qry->fetch_assoc(); // Lấy dữ liệu user từ database

			// Lưu thông tin user vào session
			foreach ($row as $key => $value) {
				if ($key != 'password' && !is_numeric($key)) { // Tránh lưu password
					$_SESSION['login_' . $key] = $value;
				}
			}

			$_SESSION['login_type'] = $row['type']; // Gán giá trị 'type' của user vào session

			// Điều hướng dựa theo loại tài khoản
			if ($_SESSION['login_type'] == 1) {
				return 1; // Admin => chuyển hướng home
			} elseif ($_SESSION['login_type'] == 2) {
				return 2; // Staff => chuyển hướng staff
			} else {
				session_unset(); // Xóa session nếu không phải admin/staff
				return 0;
			}
		} else {
			return 0; // Sai username hoặc password
		}
	}
	...
	function login2()
	{
		...
		if ($qry->num_rows > 0) {
			...
			if ($_SESSION['bio']['status'] != 1 || ($_SESSION['login_type'] != 1 && $_SESSION['login_type'] != 2)) {
